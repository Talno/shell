...
sigset_t prev;//previous masking from blocksigs();
int main() 
{
    ...
    //could put handlers in one function if not lazy
...
void blocksigs()
{
  ...
  sigprocmask(SIG_BLOCK, &sig_key, &prev);
}
/*
 * Blocks sigchld
 */
void blocksig_chld()
{
  ...
  sigprocmask(SIG_BLOCK, &sig_chld, &prev);
}
/*
...
/*
 * Handles status changes from foreground processes
 */
void fgHandler(int child_status, struct Process* fg)
{
...
void sigchld_handler(int sig)
{
  ...
  blocksigs();
  int child_status;
  pid_t prev = -1;//the previous id, used to mark pid for sigsuspend()
  while((pid = waitpid(-1, &child_status, WNOHANG | WUNTRACED | WCONTINUED)) > 0)
    {
      //check for fg/bg, handle logging as appropriate
      prev = pid;
...
struct Process* getJob(int pid_j)
{
  struct Process* temp = head->next;
  while(temp != NULL && temp->pid != pid_j)
    {
      temp = temp->next;
    }
  if(temp != NULL && temp->pid == pid_j)
    {
      return temp;
    }
...
/*
 * Copies the first arg_count strings from cmd to argv
 * Ends array with a 0 char pointer
 */
void getArgv(char** cmd, char** argv, int arg_count)
{
  int i = 0;
...
/*
 * Determines which built in command to execute
 */
void execBuiltIn(char** cmd, int cmd_i)
{
  char* user_cmd = cmd[0];
 ...
 * Presumed blocked signals to be in blocksigs() state beforehand(might be bad idea)
 */
void waitFG(struct Process* fg)
{
  int child_status;
